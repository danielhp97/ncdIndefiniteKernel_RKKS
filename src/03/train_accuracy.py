...
def data_preparation(base_dir, split="train"):
...
def ncd_preparation(dataset, i):
    ...
    for index, row in data.iterrows():
        row['Path'] = 'data/01/dataset{}/train/{}/img_dump/{}'.format(dataset_number,i ,row['File'])
    path_list = data['Path']
    counter_x = 0
    for i in path_list:
...
def get_testing_indices(kernel, general_labels_path, class1, class2):
    ...
    labels['truePath'] = 'data' + labels['truePath']
    #labels['truePath'] = labels.truePath.str(-2)
    labels_test = pd.read_csv("data/01/dataset{0}/train/{1}/img_dump/labels.csv".format(dataset_number, i))# open as dataframe iteration train labels
...
if __name__ == "__main__":
    ...
    kernel_cut = Ncd.get_testing_matrix(kernel, indices_train, indices_train, dataset_number)
